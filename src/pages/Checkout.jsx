...
const Checkout = () => {
  ...
  // =========================================================
  // DYNAMIC ITEM-BY-ITEM CALCULATIONS FOR SUBTOTAL, TAX & TOTAL
  // =========================================================
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        // Different backends return the new order's id under
        // different keys. Try the common ones in order, instead
        // of assuming response.data.id always exists.
        // ---------------------------------------------------------
        const orderId =
          response.data?.id ??
        ...
        if (!orderId) {
          // We still navigate to the success page (route now treats
          // :id as optional, so this never shows a blank page), but
          // we log this so it's easy to spot during testing that the
          // backend response didn't include an id anywhere expected.
          console.warn(
            "Order placed, but no order id was found in the response. " +
              "Check your Django order-creation view's response shape:",
            response.data
          );
